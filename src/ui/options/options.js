...
class PromptBoostOptions {
  ...
  setupLogPanel() {
    try {
      const logPanelContainer = document.getElementById('logPanelContainer');
      if (logPanelContainer && typeof LogPanel !== 'undefined') {
        this.logPanel = new LogPanel(logPanelContainer);
        this.logger.info('Log panel initialized successfully');

        // Add some initial logs to demonstrate functionality
        this.logger.debug('Log panel is now active and collecting logs');
        this.logger.info('All extension logs will appear here in real-time');
      } else {
        this.logger.warn('Log panel container not found or LogPanel class not available');
      }
    } catch (error) {
      this.logger.error('Failed to initialize log panel:', error);
    }
  }
...
let _promptBoostOptions;

// Initialize when DOM is ready
document.addEventListener('DOMContentLoaded', () => {
  _promptBoostOptions = new PromptBoostOptions();
});
